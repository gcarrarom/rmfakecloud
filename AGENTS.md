...
4. Watch the automatically triggered workflow and confirm the created tag, GitHub release, and container image all use the same version.

The workflow uses `release-it` to calculate the next version from the latest Git tag, create the release commit and tag, publish the GitHub release, and build the container image from that exact tag. Do not restore a manual dispatch trigger or add a competing version source.
